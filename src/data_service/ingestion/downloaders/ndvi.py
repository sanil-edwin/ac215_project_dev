"""NDVI - Iowa Corn Fields Only - 2016-2025 (May-Oct)"""
...
logger.info("=" * 70)
logger.info("NDVI - IOWA CORN FIELDS - 2016-2025 (MAY-OCT)")
logger.info("=" * 70)
...
# Load custom Iowa counties GeoJSON
...
# Check existing data
gcs_path = "data_raw_new/modis/ndvi/iowa_corn_ndvi_20160501_20251031.parquet"
...
logger.info(f"Will download from: {start_date} to 2025-10-31")
...
logger.info(f"Found {count} NDVI images to process")
...
for i in range(count):
    try:
        ...
        # Get appropriate corn mask (use previous year if current not available)
        mask_year = year
        while mask_year >= 2010 and mask_year not in corn_masks:
            mask_year -= 1
        
        if mask_year >= 2010 and mask_year in corn_masks:
            corn_mask = corn_masks[mask_year]
            masked_image = image.updateMask(corn_mask)
        else:
            logger.warning(f"  No mask available for {year}, skipping")
            continue
        
        # Extract stats for each county
        stats = masked_image.select('NDVI').reduceRegions(
            collection=counties,
...
# Apply scale factor (MODIS NDVI scale: 0.0001)
for col in ['mean','std','min','max']:
    if col in new_df.columns and new_df[col].notna().any():
        new_df[col] = new_df[col] * 0.0001
