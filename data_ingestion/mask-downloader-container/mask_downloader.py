...
Data Sources:
1. USDA NASS Cropland Data Layer (CDL) - Corn masks (2010-current year)
...
class MaskDownloader:
    ...
    def download_iowa_counties(self) -> gpd.GeoDataFrame:
        ...
        # Upload to GCS
        if self.gcs_manager:
            gcs_path = "data_raw/masks/iowa_counties.geojson"
    ...
    def download_cdl_year(self, year: int, iowa_counties: gpd.GeoDataFrame) -> Optional[Path]:
        ...
        Args:
            year: Year to download (2010-current)
        ...
        # Extract if we got a zip file - MEMORY EFFICIENT VERSION
        if zip_path.exists():
            extract_dir = self.temp_dir / f"cdl_{year}"
            extract_dir.mkdir(exist_ok=True)
            
            logger.info(f"Extracting {zip_path}...")
            # Extract one file at a time to avoid memory issues
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                # Find TIF file in archive first
                tif_name = None
                ...
                if not tif_name:
                    logger.error(f"No TIF file found in CDL archive for {year}")
                    return None
                
                # Extract only the TIF file (not all files at once)
                logger.info(f"Extracting {tif_name} from archive...")
                zip_ref.extract(tif_name, extract_dir)
            
            tif_path = extract_dir / tif_name
            
            # Delete zip file immediately to free up disk space
            logger.info(f"Removing zip file to free space...")
            zip_path.unlink()
            logger.info(f"Freed ~1.7 GB of disk space")
        else:
            tif_path = self.temp_dir / f"cdl_{year}.tif"
            if not tif_path.exists():
                logger.error(f"Could not find CDL TIF file for {year}")
                return None
        
        logger.info(f"Processing CDL TIF: {tif_path}")
        
        # Create Iowa corn mask
        output_path = self.temp_dir / f"iowa_corn_mask_{year}.tif"
        corn_mask_path = self.create_corn_mask(tif_path, iowa_counties, year, output_path)
        
        # Clean up extracted TIF to free memory
        if tif_path.exists():
            logger.info(f"Cleaning up extracted TIF to free space...")
            tif_path.unlink()
            if tif_path.parent.name.startswith('cdl_'):
    ...
    def create_corn_mask(
        ...
        Args:
            ...
            year: Year of data
            output_path: Path to save corn mask
        ...
        with rasterio.open(cdl_path) as src:
            ...
            # Read CDL data for Iowa extent using windowed reading (memory efficient)
            ...
            # Upload to GCS
            if self.gcs_manager:
                gcs_path = f"data_raw/masks/corn/iowa_corn_mask_{year}.tif"
    ...
    def download_all_masks(self, years: List[int] = None, start_year: int = 2010) -> dict:
        """
        ...
        Args:
            ...
            start_year: First year to download (default: 2010)
        ...
        logger.info(f"  Location: data_raw/masks/corn/")
        logger.info("="*70)
        ...
        if self.gcs_manager:
            ...
            logger.info("-" * 70)
            for year in years:
                mask_gcs_path = f"data_raw/masks/corn/iowa_corn_mask_{year}.tif"
            ...
            if not results['missing_years']:
                ...
                logger.info(f"  Location: gs://{self.gcs_manager.bucket_name}/data_raw/masks/corn/")
        ...
        counties_path = "data_raw/masks/iowa_counties.geojson"
        ...
        if self.gcs_manager:
            logger.info(f"\n  ✓ All data in: gs://{self.gcs_manager.bucket_name}/data_raw/masks/")
...
def main():
    ...
    if years_str:
        years = [int(y.strip()) for y in years_str.replace(';', ',').split(",")]
